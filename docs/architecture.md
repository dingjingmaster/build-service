...
token = change-me-agent-001
labels = linux,amd64,gcc13
enabled = true
```
...
- letting the user select target agents or agent groups/labels
...
- authenticating with its own token
- reporting name, labels, platform, concurrency, and current state
...
The UI should support selection by:

- explicit agent names
- labels
...
Each agent has an independent token.
...
- agents page: name, labels, platform, status, capacity, current runs, last seen
- upload page: upload source archive and select target agents by name or labels
